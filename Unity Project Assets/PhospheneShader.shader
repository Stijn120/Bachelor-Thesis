// Shader adapted from https://forum.unity.com/threads/how-would-i-mask-this-geometry-shader-with-a-texture.488356/

Shader "Unlit/PhospheneShader"
{
    Properties
    {
        _PhospheneShape ("PhospheneShape", 2D) = "white" {}
        _PhospheneSize ("PhospheneSize", Float) = 0.1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _PhospheneShape;
            float _PhospheneSize;
            ...
            void geom (point GS_INPUT tri[1], inout TriangleStream<FS_INPUT> triStream)
            {
                ...
                pIn.vertex = mul(UNITY_MATRIX_VP, vertex + float4(tangent * -_PhospheneSize / 1.5, 0));
                pIn.texcoord = float2(-0.5,0);
                triStream.Append(pIn);
 
                pIn.vertex = mul(UNITY_MATRIX_VP, vertex + float4(up * _PhospheneSize, 0));
            ...
            float4 frag (FS_INPUT i) : COLOR
            {
                ...
                color.a = step(0.5, tex2D(_PhospheneShape, i.texcoord).a);
                return color;
            }
